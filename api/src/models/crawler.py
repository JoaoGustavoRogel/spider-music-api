...
class ConcreteSpotifyChartsCrawler(AbstractCrawler):
    ...
    def request_data(self, params):
        ...
        url = f"https://spotifycharts.com/{chart_type}/{region}/{period}/{date}/download"
        ...
        try:
            ...
            raise Exception(f"Não foi possível baixar o arquivo {filename} na url {url}: {e}")
    ...
    def process_data(self, data, params):
        ...
        data["track_id"] = data.apply(lambda row: row["url"].split("track/")[-1], axis = 1)
    ...
    def get_data(self, params):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=16) as executor:
            future_to_info = {executor.submit(self.processing_helper, info): info for info in collect_info}
            for future in concurrent.futures.as_completed(future_to_info):
                info = future_to_info[future]
                try:
                    tidy += future.result()
                except Exception as e:
                    raise Exception(f"Não foi possível processar os dados para os seguintes parâmetros: {info}\n{e}")
    ...
    def processing_helper(self, info):
        ...
        try:
            ...
            raise Exception(f"Não foi possível processar os dados para os seguintes parâmetros: {params}\n{e}")
